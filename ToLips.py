...
class ShowImage(QMainWindow):
    ...
    def facedetect(self):
        ...
        if dominant_percentage > 3:  
            ...
            if self.tonelabel.text() == "Warna Kulit: Kuning Langsat":
                ...
                self.comboBox.addItems(["Semua Wae=rna", "Flush It Red", "Night Purple", "Crimson Love","Peach Addict", "Ambitious", "Heroine"])
            else:
    ...
    def changeLipstick(self,img, value):
        """ Mengubah warna bibir pada gambar yang terdeteksi wajahnya """   
        ...
        for face in faces:
            ...
            kernel_size = (7, 7)  # Ukuran kernel untuk blur
    ...
    def getMaskOfLips(self,img, points):
        """ Mendapatkan masker dari bibir wajah """
        mask = np.zeros_like(img)
        mask = cv2.fillPoly(mask, [points], (255, 255, 255))
        return mask

    def applyLipstick(self, face_image_path, color_values):
        """Menerapkan warna bibir pada gambar wajah menggunakan nilai warna yang diberikan"""
        face_img = cv2.imread(face_image_path)
        if face_img is None:
            print("Gambar wajah tidak dapat dibaca.")
            return

    # Resize gambar wajah menjadi setengah dari lebar maksimum 800 piksel
        face_img = imutils.resize(face_img, width=400)

    # Ambil nilai warna dari variabel color_values
